...
async def play_next(ctx):
    if queues.get(ctx.guild.id):
        ...
        await play(link=link)  # Play the next song in queue
...
async def play(ctx: commands.Context, link: str):
    ...
    if is_playing:
        # Add the song to the queue if a song is already playing
        if ctx.guild.id not in queues:
            queues[ctx.guild.id] = []
        queues[ctx.guild.id].append(link)
        await ctx.send(f"Added to the queue: {link}")
        return
...
async def queue(ctx: commands.Context):
    if ctx.guild.id not in queues or not queues[ctx.guild.id]:
        ...
        queue_list = "\n".join([f"{index + 1}. {url}" for index, url in enumerate(queues[ctx.guild.id])])
        await ctx.send(embed=discord.Embed(title="Current Queue", description=f"```{queue_list}```", color=discord.Color.green()))
